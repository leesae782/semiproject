...
<div class="container">
   <div class="row">
         ...
         <div class="page-display text-align: center;  float: left width: 100%">
            ...
            <%if(startPageNum != 1){ %>
               <li class="page-item"><a class="page-link" href="<%=kinds %>.jsp?pageNum=<%=startPageNum-1 %>&condition=<%=condition %>&keyword=<%=encodedK %>">Prev</a></li>
            <%} %>
            <%for(int i=startPageNum; i<=endPageNum; i++){ %>
               <%if(i==pageNum){ %>
                  <li class="page-item active "><a class="page-link"  href="<%=kinds %>.jsp?pageNum=<%=i %>&condition=<%=condition %>&keyword=<%=encodedK %>"><%=i %></a></li>
               <%}else{%>
                  <li class="page-item"><a class="page-link" href="<%=kinds %>.jsp?pageNum=<%=i %>&condition=<%=condition %>&keyword=<%=encodedK %>"><%=i %></a></li>
               <%} %>
            <%} %>   
            <%if(endPageNum < totalPageCount){ %>
               <li class="page-item"><a class="page-link" href="<%=kinds %>.jsp?pageNum=<%=endPageNum+1 %>&condition=<%=condition %>&keyword=<%=encodedK %>">Next</a></li>
            <%} %>
            </ul>
            </div>
         </div>
         
         <a href="${pageContext.request.contextPath }/writepage/insertform.jsp?url=<%=url%>&kinds=<%=kinds%>"><button class="btn btn-dark" style="float:right;">글쓰기</button></a>
